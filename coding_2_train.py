...
## I have used the template to modify the solution.
...
def stepUpdate(weights, featureVals, learningRate):
    ...
    #print("val=",netVal)
    observedClass = featureVals[3]
    #print(observedClass)
    # Calculate the value after StepActivation.
    actVal = stepActivation(netVal)

    # Calculate the weights and create a numpy_array
    w1 = w1 + learningRate*featureVals[0]*(observedClass-actVal)
    w2 = w2 + learningRate*featureVals[1]*(observedClass-actVal)
    w3 = w3 + learningRate*featureVals[2]*(observedClass-actVal)
...
for i in range(epoch):
    ...
    for dataPoint in andFunction:
        ...
        andWeights = stepUpdate(andWeights, dataPoint,learningRate)
...
# plotting the graph and predicting the output
...
plt.title('Plotting AND Function...')
plt.show()
